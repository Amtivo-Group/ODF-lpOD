...
A table is an instance of the lpOD C<odf_table> class, knowing that
C<odf_lpod> is an alias for C<ODF::lpOD::Table>.
...
Like any other C<odf_element>, a table may be created either from scratch
...
the context-based C<get_table_by_name> method with the name as argument. It
may be selected by its sequential position in the list of the tables belonging
...
An application may retrieve a table from any element that belongs to it, thanks
...
relatively to the end of the table. To do so, coordinates are provided as
negative values. For example, C<(-1,-1)> designates the last cell of the last
row whatever the table size.

Every table object selection method returns a null value, without error,
when the given address is out of range.

The number of rows and columns may be got using the C<odf_table> C<get_size>
method, that returns the heigth then the width.
...
C<get_row()> selects a table row as an ODF element. This method requires a
zero-based numeric value.
...
According to the same logic, each of the two instructions below returns the
...
efficient and may produce performance issues. In addition, the column-based
C<get_cell()> may fail with a warning when used in I<read optimize> mode
(see below).
...
Note that this method allows an alternative syntax; the cell position may be
...
A row object has its own C<get_cells()> method. The row based version of
